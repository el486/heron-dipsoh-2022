    ...
    handler: function (menuItem, event) {
		...
		if(infoWms.data.items.length>0){
			infoWin = new Ext.Window({
					...
					'<br><b>Resumen: </b>{6}'+
					'<br><b>Leyenda: </b><br>{7}'
					, layer.name, layerType, tiled, hasFeatureInfo, hasWFS,infoWms.data.items[0].data.title,infoWms.data.items[0].data.abstract,'<img src="'+ serverURL +'/geoserver/dipsoh/wms?TRANSPARENT=TRUE&SINGLETILE=true&SERVICE=WMS&VERSION=1.1.1&REQUEST=GetLegendGraphic&EXCEPTIONS=application%2Fvnd.ogc.se_xml&LAYER='+layer.params.LAYERS+'&FORMAT=image%2Fgif">'),
					x: 30,
					y: 100
					});
			infoWin.show();
			...
			infoWin = new Ext.Window({
					...
					"<br><b>Soporta WFS: </b>{4}" + 
					'<br><b>Leyenda: </b><br>{5}'
					, layer.name, layerType, tiled, hasFeatureInfo, hasWFS,'<img src="'+ serverURL +'/geoserver/dipsoh/wms?TRANSPARENT=TRUE&SINGLETILE=true&SERVICE=WMS&VERSION=1.1.1&REQUEST=GetLegendGraphic&EXCEPTIONS=application%2Fvnd.ogc.se_xml&LAYER='+layer.params.LAYERS+'&FORMAT=image%2Fgif">'),
					x: 30,
					y: 100
					});
			infoWin.show();
